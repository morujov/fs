...
S5b (жизненный цикл + жалобы) проверен, **PR #7 открыт**, 247 тестов.

**S7-GDPR написан, но ещё не запускался** — ветка `s7-gdpr` поверх S5b.
Закрывает вторую половину блокера №3: retention, право на удаление,
право на выгрузку. Дальше: юр. страницы (текст) и SEO.
...
11. **Cookie-баннера нет, и это надо сохранить.**
    Мы ставим только строго необходимые куки (сессия Laravel, CSRF).
    Ни одного стороннего скрипта: Google-вход — серверный редирект через
    Socialite, JS Google на страницах нет. Строго необходимые куки согласия
    не требуют, поэтому баннер не нужен.

    **Добавишь GA, GTM, Clarity, пиксель — баннер станет обязательным**,
    и это не «ещё один компонент», а худший элемент веб-интерфейса,
    который увидит каждый посетитель. Аналитика только cookieless:
    Plausible или Matomo в безкуковом режиме.

12. **Персональные данные не хранятся вечно.**
    У каждого срока в `settings` (группа `retention`) есть цель; когда цель
    отпадает, данные уходят. Где строка нужна продукту — **обезличиваем,
    а не удаляем**: `contact_reveals` держит «повтор бесплатно», но IP там
    нужен только на 24 часа.

    Добавляешь колонку с персональными данными — сразу решай её срок.
...
## Задача сейчас: прогнать S7-GDPR

Ветка `s7-gdpr`, поверх `s5b-lifecycle-reports` (PR #7).

**Что это закрывает.** Блокер №3 блюпринта («GDPR»). В блюпринте стояла
галочка «решение: cookie-баннер, право на удаление» — и не было сделано
ничего. Пять этапов мы строили защиту контактов продавца от посторонних
и параллельно вечно копили IP покупателей без способа их стереть.

**Находка, которая снимает работу: cookie-баннер не нужен.** Мы ставим
только строго необходимые куки (сессия, CSRF). Ни одного стороннего
скрипта; Google-вход идёт серверным редиректом через Socialite, JS Google
на страницах нет. Строго необходимые куки согласия не требуют.
**Ограничение:** добавите GA/GTM/Clarity — баннер станет обязательным.
Аналитика только cookieless (Plausible / Matomo без кук).

**Что в ветке:**
- `PruneOldData` — ст. 5, ограничение хранения. Сроки в `settings`.
  Ключевая идея: **обезличивание вместо удаления** там, где строка нужна
  продукту. `contact_reveals` нельзя удалять — на ней держится «повтор
  бесплатно», — но IP там нужен только на 24 часа. Связь остаётся,
  IP зануляется.
- `AccountEraser` — ст. 17. Не просто DELETE: ст. 17(3)(e) защищает
  жалобы как возможные доказательства в чужом споре.
- `DataExporter` — ст. 15 и 20, JSON.
- `PrivacyController` + страница `/mis-datos`.
- **Миграция, меняющая схему:** `listings.user_id` и
  `contact_reveals.user_id` были `NOT NULL + cascadeOnDelete`. Удаление
  аккаунта **снесло бы все объявления каскадом**, а обезличить их было
  невозможно. Теперь nullable + nullOnDelete.
- `GdprTest` (16).
...
git checkout s7-gdpr
php artisan migrate          # меняет внешние ключи — прогони на чистой БД
php artisan test
php artisan data:prune --dry-run
```

Ключевое:
- `deleting_does_not_destroy_the_listing_history` — ловит тот самый каскад
- `deleting_anonymizes_reveals_but_keeps_the_listing_counter`
- `deleting_keeps_reports_but_detaches_them` — ст. 17(3)(e)
- `a_deleted_user_can_sign_up_again_with_the_same_google_account` —
  удаление не должно быть пожизненным баном
- `the_export_shows_the_ips_we_recorded` — прятать неудобное = врать
  в политике

### Если падает

- `Cannot change column 'user_id': used in a foreign key` → миграция
  дропает FK перед `change()`; если MySQL всё равно ругается, проверь
  порядок и что БД чистая
- `Column 'user_id' cannot be null` → миграция не прогнана
- `doctrine/dbal` требуется для `->change()` → в Laravel 11+ не нужен,
  но если версия старая — поставить

**Затем:** PR в `main` (после PR #7).
